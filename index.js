...
submitExpense.addEventListener('click', e=>{
    ...
    function addExpense () {
        ...
        let balance = budgetAmount - newExpensesVal
       ...
       let listItem =  `<div class="d-flex justify-content-between  data-index="${expenseList.children.length}">
        <p>${item}</p>
        <p>${itemPrice}F</p>
        <div>
      ...
      expenseList.innerHTML += listItem;
      ...
      expensesList.push({ item, itemPrice });
    ...
    expenseList.addEventListener('click', e => {
        if (e.target.id === 'deleteItem') {
            e.preventDefault();
            let listItem = e.target.closest('.d-flex');
            listItem.remove();
        }
    });
})
...
savedExpenses.forEach((expense) => {
    const listItem = /* ... Create the HTML for each expense item ... */
/*     expenseList.innerHTML += listItem;
}); */
...
// Load budget from local storage when the page loads
const savedBudget = localStorage.getItem('budget');
const savedExpenses = JSON.parse(localStorage.getItem('expenses')) || [];
const totalExpenses = savedExpenses.reduce((total, expense) => total + parseFloat(expense.itemPrice), 0);
...
// Adding event listener to the "Submit Expense" button
...
submitExpense.addEventListener('click', (e) => {
    ...
    function addExpense() {
        ...
        const listItem = `<div class="d-flex justify-content-between" data-index="${expenseList.children.length}">
            <p>${item}</p>
            <p>${itemPrice}F</p>
            <div>
            <svg id='editItem' xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square mx-3" viewBox="0 0 16 16">
        ...
        inputExpense.value = '';
                ...
                // Update showExpenses and showBalance in the UI after adding an expense
                showExpenses.innerHTML = `${totalExpenses + newExpensesVal}F`;
                const updatedBalance = budgetAmount - (totalExpenses + newExpensesVal);
                showBalance.innerHTML = `${updatedBalance}F`;
